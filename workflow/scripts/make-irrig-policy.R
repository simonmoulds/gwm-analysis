...
if (sys.nframe() == 0L) {
  ...
  outputdir <- "results/irrigation_policy_maps"
}

if (!dir.exists(outputdir))
  dir.create(outputdir, recursive = TRUE)

## TODO This should be varied to assess the impact of policies
sf <- 0.5
...
for (i in 1:length(command_area_ids)) {
  command_area_maps[[i]] <- (
    (command_area_maps[[i]] / india_canal_frac)
    * india_canal_frac_adj
  )
}
...
##   file.path(
##     outputdir,
##     paste0("icrisat_kharif_canal_2010_india_0.500000Deg_current_canal.tif")
##   ),
##   overwrite=TRUE
## )
