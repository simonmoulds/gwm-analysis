...
setwd("~/projects/ganges-water-machine/code")
...
myplotfun4 <- function(x, ...) {
  ...
  p = ggplot(data = x, aes(x=dS_current, y=aridity, size=irr_area)) +
    ...
    ## scale_color_gradientn(colours = reds_pal(100)) +
    ## new_scale_color() +
    geom_point(data = x, aes(x=dS_restored, y=aridity, size=irr_area, color=x), shape = 1) +
    xlim(c(-0.5, 0.5)) +
    ## scale_color_gradientn(colours = blues_pal(100))
    ## scale_x_continuous(
    ##   name = "",
    ##   breaks = seq(1980, 2010, 10),
    ##   labels = c("1980", "1990", "2000", "2010"),
...
plot(df$dS_current, df$precip, col="blue")
points(df$dS_restored, df$precip, col="magenta")
## TODO size of points relate to irrigated area
## TODO shade of points relate to longitude
...
xx_igp = xx %>% filter(basin %in% "igp")
p1 <- myplotfun(xx_igp)
p1 <- p1 + ylim(c(-0.3, 0.3))

xx_igp_east = xx %>% filter(basin %in% "igp_east")
p2 <- myplotfun(xx_igp_east)
p2 <- p2  + ylim(c(-0.3, 0.3))

xx_igp_west = xx %>% filter(basin %in% "igp_west")
p3 <- myplotfun(xx_igp_west)
p3 <- p3 + ylim(c(-0.3, 0.3))

## Arrange plots
fig4 = (p1 | (p2 + p3)) + plot_layout(nrow=2)
