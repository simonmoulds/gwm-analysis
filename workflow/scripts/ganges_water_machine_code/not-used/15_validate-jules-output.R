...
## ## This adds data to a raster image:

## ## wb_output =
## ##     lapply(wb_vars, FUN=function(x) template_raster) %>%
## ##     setNames(wb_vars)

## ## for (i in 1:length(years)) {
## ##     year = years[i]
## ##     fn = paste0(id_stem, ".S2.", profile_name, ".", year, ".2D.month.nc")
## ##     nc = nc_open(file.path(jules_output_dir, fn))
## ##     for (var in wb_vars) {
## ##         arr = ncvar_get(nc, var)
## ##         arr = apply(arr, c(1,2), sum)
## ##         ## if (length(dim(arr)) == 3) {
## ##         ##     arr = arr[,,1]
## ##         ## }
## ##         wb_output[[var]] %<>% add_to_raster(arr)
## ##     }
## ##     nc_close(nc)
## ## }
