...
## sw_maps = irrigation_source_maps[[c("canal","other_sources","tanks")]]
## gw_maps = irrigation_source_maps[[c("other_wells","tubewells")]]
## sw_total = stackApply(sw_maps, indices=rep(1, 3), fun=sum)
## gw_total = stackApply(gw_maps, indices=rep(1, 2), fun=sum)
## total_irrigated_area = stackApply(irrigation_source_maps, indices=rep(1, 5), fun=sum)

library(sf)
library(dplyr)
command_areas = st_read("../data/command_areas.shp")
pakistan_ids = c(1,2,3,4,5,8,9,10,11,12,13,14,16,20,37,41)
india_command_areas = command_areas %>% filter(!ID %in% pakistan_ids)

india_frac = raster("../data/india_frac_0.500000Deg.tif")
wfdei_frac = raster("../data/WFD-EI-LandFraction2d_IGP.tif")
...
output_dir = "../data/command_area_fraction"
if (!dir.exists(output_dir)) {
    dir.create(output_dir)
}
writeRaster(
    india_canal_frac,
    file.path(output_dir, "india_command_area_fraction.tif"),
    overwrite=TRUE
)
...
for (i in 1:length(irrigation_sources)) {
    ...
    for (policy in c("current_canal", "restored_canal")) {
        for (season in c("rabi", "zaid", "continuous")) {        
            fn0 = file.path("../data/irrigated_area_maps/", paste0("icrisat_", season, "_", source, "_", reference_year, "_india_0.500000Deg.tif"))
            fn1 = file.path("../data/irrigated_area_maps/", paste0("icrisat_", season, "_", source, "_", reference_year, "_india_0.500000Deg_", policy, ".tif"))
